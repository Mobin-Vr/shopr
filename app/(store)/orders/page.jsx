...
async function Orders() {
   ...
   return (
      <div className='flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4 sm:p-8 rounded-xl shadow-lg w-full max-w-4xl'>
...
const OrderDetails = ({ order }) => (
   <div className='p-4 sm:p-6 border-b border-gray-200'>
      ...
      {order.amountDiscount && <OrderDiscount order={order} />}
   </div>
);
   ...
   <div className='px-4 py-3 sm:px-6 sm:py-4'>
      ...
      <div className='space-y-3 sm:space-y-4'>
         {order.products?.map((product) => (
            <OrderItem key={product.product?._id} product={product} />
         ))}
      </div>
   </div>
);
